...
import pandas as pd
import time
from utils.otsmb_aic import smb_retrieve_file, smb_store_file, smb_connect
import json
import os
...
metadata = {
    'apiLevel': '2.13',
    'protocolName': 'Titration',
    'description': 'This protocol is designed for acid_base titration, redox titration, and complexometric titration',
    'author': 'Biplab Dutta'
    }
...
def run(protocol: protocol_api.ProtocolContext):
    try : 
        ...
        # parsing ot2_system_config_h2o2.json file
        with open(os.path.join(path,'ot2_system_config_h2o2.json')) as f:
            data = json.load(f)
        # parsing ot2-transfer_config_h2o2.json file
        ...
        reservoir_h2o = protocol.load_labware(data['reservoir_4']['type'], data['reservoir_4']['pos'])
        # Load 96 well plate
        ...
        # Load a P20 Single GEN2 on the right mount
        right = protocol.load_instrument(data['pipette_2']['type'], data['pipette_2']['mount'], 
                                         tip_racks=[tiprack1,tiprack2,camera_rack])
        
        ######################################### transfer config ##################################################

        ## Transfer 1 : Buffer or Indicator addition(H2SO4), Multi-Channel Pipette p300
        #right.drop_tip()
        #left.drop_tip()
        ...
        print('Buffer and Indicator (H2SO4) Transfer:\n')
        count = 0
        vols = vol_transfer['transfer_1']['vol']
        value_number = 0
        for tip, vol, transfer_el in zip(vol_transfer['transfer_1']['tiprack'], vols, vol_transfer['transfer_1']['locs']): 
            el0 = transfer_el[0]
            vol = list(vols.values())[value_number]   
            print('pick up tips from deck {} {}'.format(data['tiprack_4']['pos'], tip))
            left.pick_up_tip(tiprack4[tip])
            ...
            left.drop_tip()
            value_number += 1
        print('Buffer and Indicator (H2SO4) transfer complete\n')
        print('#'*100)
         

        ## Transfer 2 : Sample Addition(H2O2), Multi-Channel Pipette p300
        print('Sample (H2O2) Transfer:\n')
        count = 0
        ...
        for tip, el0, el1 in zip(vol_transfer['transfer_2']['tiprack'],
            ...
            count +=1
            print('*'*10)     
        print('Sample (H2O2) transfer complete\n')
        print('#'*100)
    

        ## Transfer 3 : Pre-Estimation(titrant), Single-Channel Pipette p20, tiprack_2: E11-H11
        ## 'prestimation' mode
        ...
        for tip, transfer_el in zip(vol_transfer['transfer_3']['tiprack'], vol_transfer['transfer_3']['locs']):
            ...
            for el1 in transfer_el[1] : 
                # Add 20 uL   
                print('aspirating from deck {} {} dispensing {} uL to plate deck {} {}'.format(data['reservoir_3']['pos'],
                                                                                          el0,  
                                                                                          vol,
                                                                                          data['plate_1']['pos'],
                                                                                          el1
        ...
        for well in mix_col :
            ...
            left.aspirate(vol3, reservoir_h2o['A3'], rate = aspirate_speed)
            left.dispense(vol3, reservoir_h2o['A9'], rate = dispense_speed)
            ...
            left.blow_out()
        left.drop_tip()
        # trigger camera and record the pre-estimation result
        ...
        ## Transfer 4 : Titartion(titrant), Single-Channel Pipette p20, tiprack_1
        ## 'titration' mode
        print('Titration :\n')
        count = 0
        vol = vol_transfer['transfer_4']['vol']
        # multichan pick up tips 
        left.pick_up_tip(tiprack4['A4'])
        for samp_id, transfer_el in enumerate(vol_transfer['transfer_4']['locs']):
            titration_count = 0
            # run camera to image blanc plate
            samp_dict = {'id' : samp_id + 1, 
                         'mode' : 'titration', 
                         'titration_count' : 0, 
                         'status' : 'run', 
                         'cols' : transfer_el[1]}             
            # zero-time : no titrant(KMnO4)
            trigger_cam(right,
                        camera_rack,  
                        data['camera']['tip'], 
                        connection, delay = 20, 
            ...
            for k in range(vol_transfer['transfer_4']['titration'][titration_count]):
                ...
                left.blow_out()
                print('rinsing protocol2: aspirating & dispensing deck 4, A3')
                # trigger camera and record each step of titration 
                samp_dict = {'id' : samp_id + 1, 
                             'mode' : 'titration', 
                             'titration_count' : k+1, 
                             'status' : 'run', 
